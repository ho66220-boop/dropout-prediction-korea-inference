...
이 결과는 학생의 중도탈락 또는 졸업 여부가 입학 당시 정보만으로 결정되는 것이 아니라, 입학 후 학업 진행 상황과 행정적·재정적 상태가 함께 반영되는 문제임을 시사한다. 따라서 본 프로젝트는 단순 분류 모델이라기보다, 학생 지원 또는 조기경보 관점에서 해석 가능한 분석 결과를 제공한다. 다만 변수 중요도는 인과관계를 의미하지 않으며, 실제 개입 정책 설계에는 학교 현장의 도메인 검증과 추가 분석이 필요하다.
